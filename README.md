...
```sh
uv run dcl image clean                                  # 使用預設設定，只預覽
uv run dcl image clean --config /path/to/images.yaml --yes
...
預設只預覽、不詢問輸入；加上 `--yes` 才執行。`--delete` 只選命中的 image，`--keep` 保護命中的整個 image；兩者同時使用時保留優先。只有 `--keep` 時清理其餘 image。兩個參數都可重複，多條採 OR，以 Python `re.search` 比對完整 tag；任一 tag 命中即作用於整個 ID。命令列空白或無效 Regex 拒絕執行。不指定 `--delete` 時，清理未受保留規則或容器引用保護的 image；設定檔明確 `keep: []` 合法，表示沒有檔案保留規則。
